 ...
 void mcu_write_rtctime(unsigned char time[])
{
   ...
   */
    if(time[0] == 1) {
				RTC_Set(time[1]+2000,time[2],time[3],time[4],time[5],time[6]);	
				oled_time_put(time[1]+2000,time[2],time[3],time[7]);
...
//    }else 
		printf("get_wether\r\n");
		if(my_strcmp(name, "aqi") == 0) {
				wether_cooking(0,2,value_int);
        printf("day:%d aqi value is:%d\r\n", day, value_int);          //int 型
    }else if(my_strcmp(name, "condition") == 0) {
				condition_choose(value_string);
